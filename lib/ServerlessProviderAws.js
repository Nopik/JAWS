  ...
  request(service, method, params, stage, region, options) {
    ...
    return SUtils.persistentRequest(function () {
      return new BbPromise(function (resolve, reject) {
        let awsService = new _this.sdk[service](_this.getCredentials(stage, region));
  ...
  getCredentials(stage, region) {
    ...
    if (stage && process.env[this._config.envPrefix + 'AWS_PROFILE_' + stage]) {
      ...
      credentials = {
        ...
        secretAccessKey: process.env[this._config.envPrefix + 'AWS_SECRET_ACCESS_KEY_' + stage],
        region: region
      };
      ...
      credentials = {
        ...
        secretAccessKey: process.env[this._config.envPrefix + 'AWS_SECRET_ACCESS_KEY'] || process.env[this._config.envPrefix + 'SERVERLESS_ADMIN_AWS_SECRET_ACCESS_KEY'],
        region: region
      };
